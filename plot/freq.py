...
while mydata:
        ...
        try:
                config = re.search(r"(([-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?)sec_([-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?)nodes)\.freq", el ).group( 1 )
        ...
        plt.savefig( dest + config + "-apps-freq.pdf" );
        plt.close()
